import { BlockPermutation, Dimension, EntityComponentTypes, EntityEquippableComponent, EquipmentSlot, ItemStack, Player, type Vector3 } from "@minecraft/server";
import { randomNum } from "../utils";

export function summonEntity(dim: Dimension, location: Vector3, id: string, count: number = 1) {
...
export function fillArea(dim: Dimension, center: Vector3, r: number, blockId: string) {
  ...
  for (let x = center.x - r; x <= center.x + r; x++) {
    for (let y = center.y; y <= center.y + 2; y++) {
      for (let z = center.z - r; z <= center.z + r; z++) {
        try { dim.setBlockPermutation({ x, y, z }, perm); } catch (_) { }
      }
    }
  }
}

export function fillSquare(dim: Dimension, center: Vector3, r: number, yOff: number, blockId: string) {
  const perm = BlockPermutation.resolve(blockId);
  for (let x = center.x - r; x <= center.x + r; x++) {
    for (let z = center.z - r; z <= center.z + r; z++) {
      try { dim.setBlockPermutation({ x, y: center.y + yOff, z }, perm); } catch (_) { }
    }
  }
}

export function playerEffect(player: Player, effectId: string, duration: number, amplifier: number = 0, showParticles: boolean = true) {
  try {
...
export function setHelmet(player: Player, itemId: string) {
  try {
    const eq = player.getComponent(EntityComponentTypes.Equippable) as EntityEquippableComponent;
